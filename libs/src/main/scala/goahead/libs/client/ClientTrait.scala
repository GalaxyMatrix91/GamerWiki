...
import goahead.libs.http.{ CirceSupport, FormField }
...
trait ClientTrait {
  ...
  implicit val GoAheadExceptionFormat = new GoAheadExceptionFormat()
  ...
  protected def toObject[Out](resp: HttpResponse)(implicit d: Decoder[Out]): Future[Out] = {
    resp.status match {
      ...
      case StatusCodes.BadRequest => Unmarshal(resp.entity).to[GoAheadException].flatMap(ex => Future.failed(ex))
