...
import goahead.libs.model.ApiErrors
...
trait JsonSupport { self: ServerSupport =>
  ...
  final def JResult[T](data: T)(implicit d: Encoder[T]): HttpResponse = {
    ...
    HttpResponse(StatusCodes.OK, entity = HttpEntity(ContentTypes.`application/json`, Jsons.SlimPrinter.pretty(json)))
  }
  ...
  final def JResult(ex: Throwable)(implicit ctx: RequestContext): HttpResponse = {
    ...
    HttpResponse(
      ...
      entity = HttpEntity(ContentTypes.`application/json`, Jsons.SlimPrinter.pretty(json)))
  }
