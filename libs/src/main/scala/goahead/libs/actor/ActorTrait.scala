...
trait ActorTrait extends Actor with ActorLogging {
  ...
  def futureOrder[T, R](seq: List[T], delay: FiniteDuration = Duration.Zero, exitOnError: Boolean = false)(
  ...
  // 重试创建 Actor, 主要处理重名问题
  def retryActor(props: Props, name: String, maxRetry: Int): Try[ActorRef] = {
    Try {
  ...
  // 间隔周期重试
  def retryDelay[T](maxRetry: Int, delay: FiniteDuration)(f: => Future[T]): Future[T] = {
    val p = Promise[T]
    def loop(i: Int): Unit = {
      f.onComplete {
