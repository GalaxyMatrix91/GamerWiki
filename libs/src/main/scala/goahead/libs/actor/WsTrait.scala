package goahead.libs.actor

import java.time.{Instant, ZonedDateTime}

import akka.actor.ActorRef
import io.circe._
import goahead.libs.actor.WsTrait._
...
trait WsTrait { self: ActorTrait =>
  ...
  def fail[T](error: Json, tagOpt: Option[SN] = None)(
    ...
    classTag: ClassTag[T],
    d: Decoder[GoAheadException]): Future[T] = {
    val tryResult = Jsons
      .from[GoAheadException](error)
  ...
  // 发送异常消息
  def postError(action: String, ex: Throwable)(implicit e: Encoder[GoAheadException], format: WsFormat[Out]): Unit = {
    val to = format.from(action, None, Some(toJson(ex)), None)
    outRef ! to
  }

  // 异常转成 Json
  def toJson(ex: Throwable)(implicit e: Encoder[GoAheadException]): Json = ex match {
    case q: GoAheadException => Jsons.toJson(q)
